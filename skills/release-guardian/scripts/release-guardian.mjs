...
import { isAbsolute as isAbsolute4, relative as relative4, resolve as resolve5, sep as sep3 } from "node:path";
...
var CHECK_CATEGORIES = /* @__PURE__ */ new Set(["test", "typecheck", "build"]);
var TOP_LEVEL_KEYS = /* @__PURE__ */ new Set(["version", "diff", "checks", "limits"]);
...
function parseCommands(value, defaultTimeoutMs) {
  ...
  for (const [index, raw] of value.entries()) {
    ...
    assertKnownKeys(raw, /* @__PURE__ */ new Set(["id", "category", "cwd", "argv", "required", "timeoutSeconds"]), `checks.commands[${index}]`);
    const id = raw.id;
    ...
    ids.add(id);
    if (typeof category !== "string" || !CHECK_CATEGORIES.has(category)) {
    ...
    const timeoutMs = raw.timeoutSeconds === void 0 ? defaultTimeoutMs : positiveInteger(raw.timeoutSeconds, `checks.commands[${index}].timeoutSeconds`, defaultTimeoutMs) * 1e3;
    if (timeoutMs > 24 * 60 * 60 * 1e3) throw new TypeError(`checks.commands[${index}].timeoutSeconds must not exceed 86400`);
    commands.push({
      id,
      category,
      cwd,
...
function resolveConfig(base = {}, project) {
  if (project !== void 0) {
  ...
  assertKnownKeys(diff, /* @__PURE__ */ new Set(["includeUntracked", "include_untracked", "maxBytes", "max_bytes", "exclude", "generated"]), "diff");
  assertObject(checks, "checks");
  assertKnownKeys(checks, /* @__PURE__ */ new Set(["required", "discover", "commands"]), "checks");
  assertObject(discover, "checks.discover");
  assertKnownKeys(discover, /* @__PURE__ */ new Set(["maxDepth", "max_depth"]), "checks.discover");
  assertObject(limits, "limits");
  assertKnownKeys(limits, /* @__PURE__ */ new Set(["maxFiles", "max_files", "maxFindings", "max_findings", "maxCheckOutputBytes", "max_check_output_bytes", "maxChecks", "max_checks"]), "limits");
...
var GENERAL_RISK_CANDIDATE = /(?:\b(?:AKIA|ASIA)[A-Z0-9]{8,}|\bgh[pousr]_|\bgithub_pat_|\bxox[baprs]-|\bsk_live_|\bAIza|PRIVATE KEY|\bBearer\s+|\b(?:password|passwd|token|secret|api[_-]?key)\b|:\/\/[^\s:@/]{2,}:[^\s@/]{4,}@|\b(?:curl|wget|base64)\b|rejectUnauthorized|\bverify\s*[:=]|verify_ssl|ssl_verify|tls_verify|NODE_TLS_REJECT_UNAUTHORIZED|\bshell\s*=|\bos\.system\s*\(|child_process\.|Runtime\.getRuntime|\beval\s*\(|\bnew\s+Function\s*\(|ScriptEngineManager|\bexec\s*\(|\bchmod\b|\bsudo\b|\bprivileged\s*:|docker\.sock|\bsetuid\s*\(|\brm\s+-rf\b|\bDROP\s+(?:DATABASE|TABLE)\b|\bgit\s+push\b|\bterraform\s+destroy\b|Access-Control-Allow-Origin|\bcors\s*\(|\bauth\w*\s*[:=]\s*(?:false|disabled)|\bEffect\s*:\s*['"]Allow)/i;
function isRiskCandidateLine(path2, text) {
  if (GENERAL_RISK_CANDIDATE.test(text)) return true;
  if (/(^|\/)package\.json$/i.test(path2) && /"(?:preinstall|install|postinstall|prepare)"\s*:/.test(text)) return true;
  if (/(^|\/)\.github\/workflows\/.*\.ya?ml$/i.test(path2) && /permissions\s*:|(?:contents|packages|actions|id-token|pull-requests)\s*:|pull_request_target\s*:|\buses\s*:|secrets\.|pull_request|head_ref/i.test(text)) return true;
  if (/\.(?:[cm]?[jt]sx?)$/i.test(path2) && /\b(?:describe|it|test)\.skip\s*\(|\bx(?:it|describe)\s*\(/.test(text)) return true;
  if (/\.(?:java|kt|kts)$/i.test(path2) && /@Disabled\b/.test(text)) return true;
  if (/\.py$/i.test(path2) && /pytest\.mark\.skip\b/.test(text)) return true;
  return /\.rs$/i.test(path2) && /#\[ignore\]/.test(text);
}

// src/core/process.ts
var import_cross_spawn = __toESM(require_cross_spawn(), 1);
function appendTailBuffer(chunks, currentBytes, chunk, maxBytes) {
  chunks.push(chunk);
...
async function runArgv(argv, options) {
  ...
  return await new Promise((resolvePromise, rejectPromise) => {
    const child = (0, import_cross_spawn.default)(argv[0], argv.slice(1), {
      cwd: options.cwd,
    ...
    const terminate = () => {
      ...
      try {
        if (process.platform !== "win32" && child.pid !== void 0) process.kill(-child.pid, "SIGTERM");
      ...
      forceKillTimer ??= setTimeout(() => {
        try {
          if (process.platform !== "win32" && child.pid !== void 0) process.kill(-child.pid, "SIGKILL");
...
function minimalCheckEnvironment() {
  const allowed = [
    "PATH",
    "HOME",
    "USERPROFILE",
...
var EXCLUSION_SAMPLE_LIMIT = 5;
function exclusionPattern(path2, patterns) {
  return patterns.find((pattern) => minimatch(path2, pattern, { dot: true }));
}
function summarizeExclusions(files) {
  const summaries = /* @__PURE__ */ new Map();
  for (const file of files) {
    if (file.excludedBy === void 0) continue;
...
async function git(cwd, args, maxOutputBytes, signal) {
...
async function resolveMergeBase(repoRoot, base, head, signal) {
  return (await git(repoRoot, ["merge-base", "--", base, head], 64 * 1024, signal)).trim();
}
async function readTextAtRevision(repoRoot, revision, path2, maxBytes = 1024 * 1024, signal) {
  const result = await runArgv(["git", "-c", "core.pager=cat", "cat-file", "-p", "--", `${revision}:${path2}`], {
    cwd: repoRoot,
    timeoutMs: GIT_TIMEOUT_MS,
  ...
  if (/does not exist|not a valid object name|path .* exists on disk/i.test(result.stderr)) return void 0;
  throw new Error(`git cat-file failed: ${result.stderr.trim() || `exit ${String(result.exitCode)}`}`);
}
async function emptyTree(repoRoot, signal) {
  return (await git(repoRoot, ["hash-object", "-t", "tree", "--stdin"], 64 * 1024, signal)).trim();
}
function decodeGitPath(input) {
...
function parseNameStatus(raw) {
  ...
  for (let index = 0; index < fields.length; ) {
    ...
    if (code.startsWith("R") || code.startsWith("C")) {
      ...
      files.push({ path: path3, oldPath, status: "renamed", changeStatus: "renamed", contentKind: "text", addedLines: 0, deletedLines: 0 });
      continue;
    }
    const path2 = fields[index++] ?? "";
    const status = code === "A" ? "added" : code === "D" ? "deleted" : "modified";
    files.push({ path: path2, status, changeStatus: status, contentKind: "text", addedLines: 0, deletedLines: 0 });
  }
  return files;
}
function* iterateLines(raw) {
  let start = 0;
...
function parsePatch(raw, files, orderedFiles) {
  const addedLines = [];
  ...
  for (const rawLine of iterateLines(raw)) {
    if (rawLine.startsWith("diff --git ")) {
      ...
      currentPath = next !== void 0 && byPath.has(next.path) ? next.path : void 0;
      currentOldPath = next !== void 0 && byPath.has(next.path) ? next.oldPath ?? next.path : void 0;
      continue;
    }
    if (rawLine.startsWith("--- ")) {
      const marker = rawLine.slice(4);
      currentOldPath = marker === "/dev/null" ? void 0 : decodeGitPath(marker.startsWith("a/") ? marker.slice(2) : marker);
      continue;
    }
    if (rawLine.startsWith("+++ ")) {
      const marker = rawLine.slice(4);
      currentPath = marker === "/dev/null" ? currentOldPath : decodeGitPath(marker.startsWith("b/") ? marker.slice(2) : marker);
      continue;
    }
    ...
    if (rawLine.startsWith("+") && !rawLine.startsWith("+++")) {
      if (currentPath !== void 0) {
        ...
        if (file !== void 0) {
          ...
          file.addedLines += 1;
          if (file.excludedBy === void 0) added += 1;
        }
      }
      newLine += 1;
      continue;
    ...
    if (rawLine.startsWith("-") && !rawLine.startsWith("---")) {
      if (currentPath !== void 0) {
        ...
        if (file !== void 0) {
          file.deletedLines += 1;
          if (file.excludedBy === void 0) deleted += 1;
        }
      }
      continue;
    }
    if (rawLine.startsWith("Binary files ") && currentPath !== void 0) {
      const file = byPath.get(currentPath);
      if (file !== void 0) {
        file.status = "binary";
...
async function collectUntracked(repoRoot, remainingBytes, remainingFiles, exclude, signal) {
  const raw = await git(repoRoot, ["ls-files", "--others", "--exclude-standard", "-z", "--"], Math.max(64 * 1024, remainingBytes + 1), signal);
  const paths = raw.split("\0").filter((path2) => path2.length > 0);
  const files = [];
  const lines = [];
  const diagnostics = [];
  const fingerprint = createHash("sha256").update(paths.join("\0"));
  const excludedCandidates = paths.flatMap((path2) => {
    const excludedBy = exclusionPattern(path2, exclude);
    return excludedBy === void 0 ? [] : [{ path: path2, excludedBy }];
  });
  let usedBytes = 0;
  let includedAddedLines = 0;
  let truncated = paths.length > remainingFiles;
  for (const path2 of paths.slice(0, remainingFiles)) {
    const excludedBy = exclusionPattern(path2, exclude);
    const absolute = resolve2(repoRoot, path2);
    const fileStat = await lstat(absolute);
    if (fileStat.isSymbolicLink() || !fileStat.isFile()) {
      diagnostics.push(`untracked path was not scanned because it is not a regular file: ${path2}`);
      truncated = true;
      continue;
    }
    const actual = await realpath(absolute);
    if (!inside(repoRoot, actual)) {
      diagnostics.push(`untracked path escapes the repository root: ${path2}`);
      truncated = true;
      continue;
    }
    if (fileStat.size > remainingBytes - usedBytes) {
      files.push({ path: path2, status: "binary", changeStatus: "untracked", contentKind: "uninspected", addedLines: 0, deletedLines: 0, ...excludedBy === void 0 ? {} : { excludedBy } });
      truncated = true;
      continue;
    }
    const content = await readFile(actual);
    fingerprint.update("\0").update(path2).update("\0").update(content);
    usedBytes += content.length;
    if (content.includes(0)) {
      files.push({ path: path2, status: "binary", changeStatus: "untracked", contentKind: "binary", addedLines: 0, deletedLines: 0, ...excludedBy === void 0 ? {} : { excludedBy } });
      continue;
    }
    const text = content.toString("utf8");
    const fileLines = text.split(/\r?\n/);
    if (fileLines.at(-1) === "") fileLines.pop();
    files.push({ path: path2, status: "untracked", changeStatus: "untracked", contentKind: "text", addedLines: fileLines.length, deletedLines: 0, ...excludedBy === void 0 ? {} : { excludedBy } });
    if (excludedBy === void 0) includedAddedLines += fileLines.length;
    for (const [index, line] of fileLines.entries()) {
      if (isRiskCandidateLine(path2, line)) lines.push({ path: path2, line: index + 1, text: line });
    }
  }
  return {
    files,
...
async function collectDiff(repoRoot, options) {
  const exclude = options.exclude ?? [];
  let base = null;
  let resolvedHead = "unborn";
  let diffTail;
  if (options.mode === "range") {
    if (options.base === void 0 || options.base.length === 0) throw new TypeError("range mode requires a base ref");
  ...
  const nameStatus = await git(repoRoot, [...flags, "--name-status", "-z", ...diffTail, "--"], Math.max(options.maxBytes, options.maxFiles * 4096), options.signal);
  const orderedFiles = parseNameStatus(nameStatus);
  for (const file of orderedFiles) {
    const excludedBy = exclusionPattern(file.path, exclude);
    if (excludedBy !== void 0) file.excludedBy = excludedBy;
  }
  ...
  const allExcludedCandidates = orderedFiles.flatMap(
    (file) => file.excludedBy === void 0 ? [] : [{ path: file.path, excludedBy: file.excludedBy }]
  );
  ...
  const fingerprint = `sha256:${createHash("sha256").update(options.mode).update("\0").update(base ?? "").update("\0").update(resolvedHead).update("\0").update(nameStatus).update("\0").update(patch.stdout).update("\0").update(untrackedFingerprint).digest("hex")}`;
  return {
    mode: options.mode,
    base,
    head: options.head,
    ...
    addedLines,
    filesChanged,
    filesSeen: files.filter((file) => file.excludedBy === void 0).length,
    filesExcluded: allExcludedCandidates.length,
    filesUnseen: Math.max(0, filesChanged - files.filter((file) => file.excludedBy === void 0).length - allExcludedCandidates.length),
    exclusions,
    fingerprint,
...
import { access, readdir, readFile as readFile2, realpath as realpath2, stat } from "node:fs/promises";
import { basename, dirname, isAbsolute as isAbsolute3, join as join2, relative as relative3, resolve as resolve3, sep as sep2 } from "node:path";
...
var INTRINSIC_DISCOVERY_EXCLUDE = [
  "**/.git/**",
  "**/.claude/worktrees/**",
  "**/.codex/worktrees/**",
  "**/.venv/**",
...
var DISCOVERY_GIT_OUTPUT_BYTES = 64 * 1024 * 1024;
...
function relativeToRoot(repoRoot, path2) {
  const result = relative3(repoRoot, path2);
  if (result === "") return ".";
  if (isAbsolute3(result) || result === ".." || result.startsWith(`..${sep2}`)) return void 0;
  return slashPath(result);
}
function isManifestName(name) {
  const lower = name.toLowerCase();
  return MANIFEST_NAMES.has(name) || name === "build.gradle" || name.startsWith("build.gradle.") || lower.endsWith(".sln") || lower.endsWith(".csproj");
}
function isExcluded(relativePath, patterns, directory) {
...
function cwdDepth(relativeCwd) {
  return relativeCwd === "." ? 0 : relativeCwd.split("/").length;
}
function commandId(relativeCwd, argv, manifestFingerprint) {
  return sha256(`${relativeCwd}\0${argv.join("\0")}\0${manifestFingerprint}`);
}
function commandKey(category, cwd, argv) {
  return `${category}\0${cwd}\0${argv.join("\0")}`;
}
function configuredFingerprint(command) {
...
async function containedFile(path2, repoRoot, visiblePaths) {
  try {
    const requestedRelative = relativeToRoot(repoRoot, path2);
    if (visiblePaths !== void 0 && (requestedRelative === void 0 || !visiblePaths.has(requestedRelative))) return false;
    const resolved = await realpath2(path2);
    if (relativeToRoot(repoRoot, resolved) === void 0) return false;
    return (await stat(resolved)).isFile();
  } catch {
    return false;
  }
}
async function gitVisiblePaths(repoRoot, diagnostics) {
  ...
  if (result.exitCode !== 0) {
    if (/not a git repository/i.test(result.stderr)) return void 0;
...
async function scanGitVisibleManifests(repoRoot, paths, maxDepth, exclude, diagnostics) {
  const manifests = [];
  ...
  for (const relativePath of [...paths].sort((left, right) => left.localeCompare(right, "en"))) {
    ...
    const requestedPath = join2(repoRoot, relativePath);
    let manifestPath;
    let bytes;
    try {
      manifestPath = await realpath2(requestedPath);
      const manifestRelative = relativeToRoot(repoRoot, manifestPath);
      if (manifestRelative === void 0) {
      ...
      bytes = await readFile2(manifestPath);
    } catch (error) {
    ...
    if (actualRelativeCwd === void 0) continue;
...
async function scanManifests(repoRoot, maxDepth, exclude, diagnostics, warnings) {
  const manifests = [];
  const visitedDirectories = /* @__PURE__ */ new Set();
  const visitedManifests = /* @__PURE__ */ new Set();
  const walk = async (requestedDirectory, depth) => {
    let directory;
    try {
      directory = await realpath2(requestedDirectory);
      if (relativeToRoot(repoRoot, directory) === void 0) {
    ...
    for (const entry of entries) {
      const requestedPath = join2(directory, entry.name);
      const requestedRelative = relativeToRoot(repoRoot, requestedPath);
      if (requestedRelative === void 0) continue;
      ...
      if (entry.isSymbolicLink()) {
        ...
        let targetStat;
        try {
          target = await realpath2(requestedPath);
          const targetRelative = relativeToRoot(repoRoot, target);
          if (targetRelative === void 0) {
      ...
      if (relativeCwd === void 0 || relativePath === void 0) continue;
...
async function detectPackageManager(cwd, repoRoot, packageManagerField, visiblePaths) {
  let current = cwd;
  while (relativeToRoot(repoRoot, current) !== void 0) {
    for (const [lockName, manager] of LOCK_FILES) {
      if (await containedFile(join2(current, lockName), repoRoot, visiblePaths)) return manager;
    }
...
function packageScriptArgv(manager, script) {
  switch (manager) {
    case "pnpm":
      return ["pnpm", "--offline", "run", script];
    case "yarn":
      return ["yarn", "--offline", "run", script];
    case "bun":
      return ["bun", "run", script];
    case "npm":
      return ["npm", "--offline", "run", script];
  }
}
async function wrapperCommand(cwd, unixName, windowsName, fallback, repoRoot, visiblePaths) {
  if (process.platform === "win32" && await containedFile(join2(cwd, windowsName), repoRoot, visiblePaths)) return `.\\${windowsName}`;
  if (await containedFile(join2(cwd, unixName), repoRoot, visiblePaths)) return `./${unixName}`;
  return fallback;
}
...
function safeProjectArgument(name) {
  return name.startsWith(".") ? name : `./${name}`;
}
async function pythonCommand(cwd, repoRoot) {
  const executableNames = process.platform === "win32" ? [".venv/Scripts/python.exe", "venv/Scripts/python.exe"] : [".venv/bin/python", "venv/bin/python"];
  let current = cwd;
  while (relativeToRoot(repoRoot, current) !== void 0) {
    for (const executableName of executableNames) {
      const candidate = join2(current, ...executableName.split("/"));
      try {
        if (!(await stat(candidate)).isFile()) continue;
        await access(candidate, constants.X_OK);
        const relativeExecutable = slashPath(relative3(cwd, candidate));
        return relativeExecutable.startsWith("../") || relativeExecutable.startsWith("./") ? relativeExecutable : `./${relativeExecutable}`;
      } catch {
      }
    }
    if (current === repoRoot) break;
...
async function discoverChecksDetailed(repoRoot, config, categories2) {
  const diagnostics = [];
  const warnings = [];
  ...
  let sequence = 0;
  const addCandidate = (target, category, cwd, relativeCwd, argv, detectedBy, required, fingerprint, manifestPath, timeoutMs) => {
  ...
  for (const command of config.commands) {
    ...
    const requestedCwd = resolve3(rootReal, command.cwd);
    let cwd;
    try {
      cwd = await realpath2(requestedCwd);
      const relativeCwd = relativeToRoot(rootReal, cwd);
      if (relativeCwd === void 0) {
  ...
  const manifests = visiblePaths === void 0 ? await scanManifests(rootReal, config.manifestDepth, discoveryExclude, diagnostics, warnings) : await scanGitVisibleManifests(rootReal, visiblePaths, config.manifestDepth, discoveryExclude, diagnostics);
  for (const manifest of manifests) {
    const required = (category) => requiredCategories.has(category);
    const detectedBy = (detail) => detail === void 0 ? manifest.relativePath : `${manifest.relativePath}#${detail}`;
    const add = (category, argv, detail) => {
      addCandidate(
    ...
    if (manifest.name === "package.json") {
      ...
      for (const category of categories2) {
        if ((category === "test" || category === "typecheck" || category === "build") && typeof scripts[category] === "string" && scripts[category].length > 0) {
...
var RULE_BY_ID = new Map(RULES.map((rule) => [rule.id, rule]));
var MANDATORY_RULES = /* @__PURE__ */ new Set(["RG001", "RG002", "RG003", "RG004", "RG005"]);
function sha2562(value) {
  return `sha256:${createHash3("sha256").update(value).digest("hex")}`;
}
function isExcluded2(path2, config) {
  return config.exclude.some((pattern) => minimatch(path2, pattern, { dot: true }));
}
function isGenerated(path2, config) {
  return config.generated.some((pattern) => minimatch(path2, pattern, { dot: true }));
}
function finding(ruleId, path2, line, evidence, message, secretValue, options = {}) {
  const rule = RULE_BY_ID.get(ruleId);
  if (rule === void 0) throw new Error(`unknown rule ${ruleId}`);
  return {
    ruleId,
    ...
    fingerprint: sha2562(secretValue ?? `${ruleId}\0${path2}\0${String(line)}\0${evidence}`),
    remediation: rule.remediation
  };
}
var PLACEHOLDERS = /^(?:example|sample|test|dummy|changeme|replace[_-]?me|your[_-]?(?:token|key|secret)|<[^>]+>|\$\{[^}]+\})$/i;
...
function scanSecretLine(path2, line, text) {
  const results = [];
  for (const pattern of STRONG_SECRETS) {
    pattern.regex.lastIndex = 0;
    for (const match2 of text.matchAll(pattern.regex)) {
      const value = match2[0];
      if (looksSynthetic(value)) continue;
      results.push(finding("RG001", path2, line, `${pattern.label} literal (${String(value.length)} characters; redacted)`, void 0, value));
    }
  }
  if (/-----BEGIN (?:RSA |EC |OPENSSH |DSA )?PRIVATE KEY-----/.test(text)) {
    const marker = /-----BEGIN [^-]+PRIVATE KEY-----/.exec(text)?.[0] ?? "private key header";
    results.push(finding("RG002", path2, line, "private key header (redacted)", void 0, marker));
  }
  const bearer = /\bBearer\s+([A-Za-z0-9._~+\/-]{20,})\b/gi.exec(text);
  if (bearer?.[1] !== void 0 && !looksSynthetic(bearer[1])) {
    results.push(finding("RG004", path2, line, `Bearer credential (${String(bearer[1].length)} characters; redacted)`, void 0, bearer[1]));
  }
  const urlCredential = /\b[a-z][a-z0-9+.-]*:\/\/([^\s:@/]{2,}):([^\s@/]{4,})@/i.exec(text);
  if (urlCredential?.[0] !== void 0 && !looksSynthetic(urlCredential[2] ?? "")) {
    results.push(finding("RG004", path2, line, "URL with embedded credentials (redacted)", void 0, urlCredential[0]));
  }
  const assignment = /\b(?:password|passwd|token|secret|api[_-]?key)\b\s*[:=]\s*['"]([^'"\s]{12,})['"]/i.exec(text);
  if (assignment?.[1] !== void 0 && !looksSynthetic(assignment[1])) {
    const unique = new Set(assignment[1]).size;
    if (unique >= 6) results.push(finding("RG003", path2, line, `probable secret assignment (${String(assignment[1].length)} characters; redacted)`, void 0, assignment[1]));
  }
  return results;
}
function redactEvidence(text) {
  ...
  redacted = redacted.replace(/\b(Bearer\s+)[A-Za-z0-9._~+\/-]{20,}\b/gi, "$1[REDACTED]").replace(/\b([a-z][a-z0-9+.-]*:\/\/)[^\s:@/]{2,}:[^\s@/]{4,}@/gi, "$1[REDACTED]@").replace(/(\b(?:password|passwd|token|secret|api[_-]?key)\b\s*[:=]\s*['"])[^'"\s]{12,}(['"])/gi, "$1[REDACTED]$2");
  return redacted;
}
function compactEvidence(text) {
  return redactEvidence(text).trim().replace(/\s+/g, " ").slice(0, 220);
}
function lineFindings(path2, line, text, context) {
  const results = scanSecretLine(path2, line, text);
  const add = (id, test, message) => {
    if (test) results.push(finding(id, path2, line, compactEvidence(text), message));
  };
  const workflow = /^\.github\/workflows\/.*\.ya?ml$/i.test(path2);
  const packageManifest = /(^|\/)package\.json$/.test(path2);
  ...
  add("RG303", /(?:^|[^\w.])eval\s*\(|\bnew\s+Function\s*\(|\bScriptEngineManager\b/.test(text) || /\.py$/i.test(path2) && /(?:^|[^\w.])exec\s*\(/.test(text));
  ...
  const skipAdded = /\.(?:[cm]?[jt]sx?)$/i.test(path2) ? /\b(?:describe|it|test)\.skip\s*\(|\bx(?:it|describe)\s*\(/.test(text) : /\.(?:java|kt|kts)$/i.test(path2) ? /@Disabled\b/.test(text) : /\.py$/i.test(path2) ? /pytest\.mark\.skip\b/.test(text) : /\.rs$/i.test(path2) && /#\[ignore\]/.test(text);
  add("RG402", skipAdded);
...
function isSensitivePath(path2) {
  const name = path2.toLowerCase();
  return /(^|\/)(?:\.env(?:\..*)?|id_(?:rsa|dsa|ecdsa|ed25519)|credentials?(?:\.[^/]*)?|secrets?(?:\.[^/]*)?|.*\.(?:pem|p12|pfx|jks|keystore))$/.test(name);
}
function isDependencyPath(path2) {
  return /(^|\/)(?:package\.json|package-lock\.json|pnpm-lock\.yaml|yarn\.lock|bun\.lockb?|pyproject\.toml|poetry\.lock|Pipfile(?:\.lock)?|requirements[^/]*\.txt|go\.(?:mod|sum)|Cargo\.(?:toml|lock)|pom\.xml|build\.gradle(?:\.kts)?|packages\.lock\.json)$/.test(path2);
}
function isReleasePath(path2) {
  return /(^|\/)(?:\.release-guardian\.ya?ml|\.github\/workflows\/[^/]*(?:release|publish|deploy)[^/]*\.ya?ml|Dockerfile|docker-compose[^/]*\.ya?ml|\.npmrc|\.pypirc|Chart\.yaml|.*\.(?:tf|hcl))$/i.test(path2);
}
function isSchemaPath(path2) {
  return /(^|\/)(?:migrations?|schema|openapi|public-api)(?:\/|\.|$)|\.(?:sql|proto)$|openapi\.ya?ml$/i.test(path2);
}
function isTestPath(path2) {
  return /(^|\/)(?:test|tests|__tests__)(?:\/|$)|(?:^|\/)[^/]*(?:\.test|\.spec|_test)\.[^/]+$/i.test(path2);
}
function findingContext(path2, config) {
  if (isGenerated(path2, config)) return "generated";
  if (/(^|\/)\.github\/workflows\/.*\.ya?ml$/i.test(path2)) return "ci";
  if (/(^|\/)(?:fixtures?|testdata|__snapshots__)(?:\/|$)/i.test(path2)) return "fixture";
  if (isTestPath(path2)) return "test";
  if (/(^|\/)(?:docs?|documentation)(?:\/|$)|(^|\/)README(?:\.[^/]*)?$|\.(?:md|mdx|rst|adoc)$/i.test(path2)) return "documentation";
  if (/(^|\/)examples?(?:\/|$)/i.test(path2)) return "example";
  if (isDependencyPath(path2) || /(^|\/)\.npmrc$/i.test(path2)) return "dependency";
  return "production";
}
var OPERATIONAL_RULES = /* @__PURE__ */ new Set(["RG103", "RG301", "RG302", "RG303", "RG304", "RG305", "RG306"]);
function clearlyInertEvidence(text, context) {
...
function safeCleanupTarget(text) {
...
function destructiveRootTarget(text) {
  return /\brm\s+-rf\s+(?:--no-preserve-root\s+)?["']?(?:\/|~|\$HOME|\$\{HOME\}|\.\.?)(?:["']?(?:\s|[,;}]|$)|\/\*)/i.test(text);
}
function adjudicateFinding(item, text, context) {
  const rationale = [...item.rationale, `file context: ${context}`];
  let result = { ...item, context, rationale };
  ...
  if (item.ruleId === "RG304" && /\bsudo\b/i.test(text) && !/\b(?:privileged\s*:\s*true|setuid\s*\(|chmod\s+(?:-R\s+)?777|rm\s+-rf)\b/i.test(text) && !/\/var\/run\/docker\.sock\s*:/.test(text)) {
...
function adjudicateFileFinding(item) {
...
function aggregateFindings(items) {
  const aggregateRules = /* @__PURE__ */ new Set(["RG304", "RG305", "RG402"]);
  const aggregated = /* @__PURE__ */ new Map();
  for (const item of items) {
    const key = aggregateRules.has(item.ruleId) ? `${item.path}\0${item.ruleId}\0${item.severity}\0${item.disposition}\0${item.context}` : `${item.path}\0${String(item.line)}\0${item.ruleId}\0${item.fingerprint}`;
    ...
    aggregated.set(key, {
      ...
      fingerprint: sha2562(fingerprints.join("\0")),
      rationale: [.../* @__PURE__ */ new Set([...previous.rationale, `aggregated ${String(previous.occurrences + item.occurrences)} matching occurrences in this file`])]
    });
  }
  return [...aggregated.values()];
}
function scanDiff(snapshot, config) {
  const findings = /* @__PURE__ */ new Map();
  const diagnostics = [];
  let findingLimitReached = false;
  const severityRank = { critical: 5, high: 4, medium: 3, low: 2, info: 1 };
  const dispositionRank = { block: 3, review: 2, inform: 1 };
  const keyOf = (item) => {
    const secretRule = ["RG001", "RG002", "RG003", "RG004"].includes(item.ruleId);
    return secretRule ? `${item.path}\0${String(item.line)}\0${item.fingerprint}` : `${item.path}\0${String(item.line)}\0${item.ruleId}\0${item.fingerprint}`;
  };
  const priorityOf = (item) => dispositionRank[item.disposition] * 10 + severityRank[item.severity];
  const pathStates = /* @__PURE__ */ new Map();
  const stateFor = (path2) => {
    const cached = pathStates.get(path2);
    if (cached !== void 0) return cached;
    const excluded = isExcluded2(path2, config);
    const state = { context: findingContext(path2, config), excluded, linePolicyExcluded: excluded || isGenerated(path2, config) };
    pathStates.set(path2, state);
    return state;
  };
  const push = (item) => {
    ...
    for (const entry of findings.entries()) {
      if (lowest === void 0 || priorityOf(entry[1]) < priorityOf(lowest[1])) lowest = entry;
    }
    if (lowest !== void 0 && priorityOf(item) > priorityOf(lowest[1])) {
  ...
  for (const file of snapshot.files) {
    ...
    const fileFinding = (ruleId, evidence) => adjudicateFileFinding(finding(
      ruleId,
      file.path,
      null,
      evidence,
  ...
  if (snapshot.truncated || findingLimitReached) {
    ...
    if (!findings.has(truncationKey)) {
      if (findings.size < config.maxFindings) findings.set(truncationKey, truncation);
        ...
        const replaceable = [...findings.entries()].filter(([, item]) => item.disposition !== "block").sort((left, right) => priorityOf(left[1]) - priorityOf(right[1]))[0];
        if (replaceable !== void 0) {
...
var OUTPUT_SECRET_ASSIGNMENT = /((?:["'])?(?:api[_-]?key|access[_-]?key|secret[_-]?key|client[_-]?secret|auth[_-]?token|access[_-]?token|refresh[_-]?token|token|secret|password|passwd|database[_-]?url|db[_-]?url)(?:["'])?\s*[:=]\s*)(?:"[^"\r\n]*"|'[^'\r\n]*'|[^\s,;]+)/gi;
var OUTPUT_URL_CREDENTIAL = /(https?:\/\/)[^/\s:@]+:[^@\s/]+@/gi;
...
function pending(plan, approved) {
  return {
    ...plan,
...
async function executeChecks(repoRoot, plans, config, runApproved, approvedCommandIds, signal) {
  ...
  for (const plan of plans) {
    ...
    try {
      const processResult = await runArgv(plan.argv, {
        cwd: resolve4(repoRoot, plan.cwd),
        timeoutMs: plan.timeoutMs ?? config.checkTimeoutMs,
        maxOutputBytes: config.maxCheckOutputBytes,
        ...signal === void 0 ? {} : { signal },
        env: minimalCheckEnvironment()
      });
      const status = processResult.aborted ? "unavailable" : processResult.timedOut ? "timed_out" : processResult.exitCode === 0 ? "passed" : "failed";
      results.push({
        ...plan,
      ...
      results.push({
        ...
        exitCode: null,
        stdoutTail: null,
        stderrTail: redactOutput(unavailable ? `command unavailable: ${plan.argv[0] ?? ""}` : String(error.message)),
        outputTruncated: false
      });
    }
  }
...
// src/core/verdict.ts
var RISK = { critical: 100, high: 30, medium: 10, low: 2, info: 0 };
var CONFIDENCE_WEIGHT = { high: 1, medium: 0.6, low: 0.25 };
function determineVerdict(findings, checks, options) {
  const reasons = [];
  const add = (reason) => {
...
// src/core/audit.ts
function summary(findings, checks) {
...
function emptyFailure(request, started, message) {
  const mode = request.mode ?? (request.base === void 0 ? "worktree" : "range");
  return {
    schemaVersion: SCHEMA_VERSION,
    toolVersion: TOOL_VERSION,
    verdict: { status: "inconclusive", riskScore: 0, reasons: ["scan-incomplete"] },
    repository: { root: resolve5(request.repoPath), head: null, base: request.base ?? null, dirty: false },
...
async function auditRelease(request, baseConfig = {}) {
  const started = Date.now();
  try {
    const repoRoot = await resolveRepositoryRoot(request.repoPath, request.signal);
    const mode = request.mode ?? (request.base === void 0 ? "worktree" : "range");
    const headRef = request.head ?? "HEAD";
    const requestedConfig = resolve5(repoRoot, request.configPath ?? ".release-guardian.yml");
    const configRelativeNative = relative4(repoRoot, requestedConfig);
    if (configRelativeNative === "" || isAbsolute4(configRelativeNative) || configRelativeNative === ".." || configRelativeNative.startsWith(`..${sep3}`)) {
      throw new TypeError("configuration path escapes the repository root or is not a file path");
    }
    const configRelative = configRelativeNative.split(sep3).join("/");
    const baselineRef = mode === "range" ? await resolveMergeBase(repoRoot, request.base ?? "", headRef, request.signal) : await resolveHeadOrNull(repoRoot, request.signal);
    const baselineConfigText = baselineRef === null ? void 0 : await readTextAtRevision(repoRoot, baselineRef, configRelative, 1024 * 1024, request.signal);
    const project = baselineConfigText === void 0 ? void 0 : parseProjectConfig(baselineConfigText);
    const config = resolveConfig({ ...DEFAULT_CONFIG, ...baseConfig, workspaceRoot: repoRoot }, project);
    if (request.maxDiffBytes !== void 0) config.maxDiffBytes = request.maxDiffBytes;
    if (request.includeUntracked !== void 0) config.includeUntracked = request.includeUntracked;
    const snapshot = await collectDiff(repoRoot, {
      mode,
      ...request.base === void 0 ? {} : { base: request.base },
      head: headRef,
    ...
    const categories2 = request.categories ?? ["test", "typecheck", "build"];
    const discovery = await discoverChecksDetailed(repoRoot, config, categories2);
    const authorizationContext = `sha256:${createHash4("sha256").update(JSON.stringify({
      repoRoot,
    ...
    const plans = discovery.plans.map((plan) => ({
      ...plan,
      id: `sha256:${createHash4("sha256").update(`${plan.id}\0${authorizationContext}`).digest("hex")}`,
    ...
    if (!discoveryScopeMatchesDiff && categories2.length > 0) {
    ...
    return {
      ...
      files: snapshot.files.filter((file) => file.excludedBy === void 0),
...
function toJsonReport(report) {
  return {
    ...
    files: report.files.map((file) => ({
      path: file.path,
      status: file.status,
      ...file.changeStatus === void 0 ? {} : { change_status: file.changeStatus },
      ...file.contentKind === void 0 ? {} : { content_kind: file.contentKind },
      added_lines: file.addedLines,
      deleted_lines: file.deletedLines,
      ...file.oldPath === void 0 ? {} : { old_path: file.oldPath },
      ...file.excludedBy === void 0 ? {} : { excluded_by: file.excludedBy }
    })),
    ...
    checks: report.checks.map((check) => ({
      ...
      ...check.authorizationContext === void 0 ? {} : { authorization_context: check.authorizationContext },
...
function formatText(report) {
  ...
  if (report.findings.length > 0) {
    lines.push("", "Findings");
    const dispositionRank = { block: 0, review: 1, inform: 2 };
    const severityRank = { critical: 0, high: 1, medium: 2, low: 3, info: 4 };
    const displayed = [...report.findings].sort(
      (left, right) => dispositionRank[left.disposition] - dispositionRank[right.disposition] || severityRank[left.severity] - severityRank[right.severity] || left.path.localeCompare(right.path) || (left.line ?? 0) - (right.line ?? 0)
    );
...
// src/cli.ts
var HELP = `dsh-release-guardian — local, read-only release risk checks
...
function positiveInteger2(value, flag) {
...
function parseCheckOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    switch (flag) {
      ...
      case "--include-untracked":
        includeUntracked = parseBoolean(value, flag);
        break;
      case "--config":
        configPath = value;
        break;
      ...
      case "--fail-on":
        ...
        break;
      case "--max-diff-bytes":
        maxDiffBytes = positiveInteger2(value, flag);
        break;
      case "--timeout":
        baseConfig.checkTimeoutMs = positiveInteger2(value, flag) * 1e3;
        break;
      default:
  ...
  if (base !== void 0 && mode !== void 0 && mode !== "range") throw new UsageError("--base conflicts with a non-range --mode");
  if (mode === "range" && base === void 0) throw new UsageError("range mode requires --base");
  const request = {
    repoPath,
    ...mode === void 0 ? {} : { mode },
    ...base === void 0 ? {} : { base },
    ...head === void 0 ? {} : { head },
    ...includeUntracked === void 0 ? {} : { includeUntracked },
    ...configPath === void 0 ? {} : { configPath },
    ...categories2 === void 0 ? {} : { categories: categories2 },
    ...maxDiffBytes === void 0 ? {} : { maxDiffBytes }
  };
  return { request, format, ...output === void 0 ? {} : { output }, runChecks, yes, failOn, baseConfig, selectedCheckIds };
}
async function confirmChecks(checks, yes) {
...
async function checkCommand(args) {
  ...
  if (options.runChecks) {
    ...
    const approvedChecks = options.selectedCheckIds.length === 0 ? report.checks : options.selectedCheckIds.map((id) => {
      const check = checksById.get(id);
      if (check === void 0) throw new UsageError(`--check-id was not found in the current plan: ${id}`);
      return check;
    });
  ...
  if (options.output === void 0) process.stdout.write(rendered);
  else await writeFile(options.output, rendered, { encoding: "utf8", flag: "wx", mode: 384 });
...
`);
  ...
  if (command === "explain") {
    ...
    if (rule === void 0) throw new UsageError(`unknown rule: ${args[0] ?? ""}`);
...
`);
    if (process.exitCode === void 0) process.exitCode = error.message === "" ? 0 : 64;
  } else {
